...
<body>
    ...
    $work_end_time = 1080;
    // 19:00 in minutes
    $overtime_start_time = 1140;
    // 22:00 in minutes
    $late_night_start_time = 1320;
    ...
    // 0時0分からの経過分数を計算
    $check_in_minutes = ($check_in_timestamp - strtotime($date . " 00:00:00")) / 60;
    $check_out_minutes = ($check_out_timestamp - strtotime($date . " 00:00:00")) / 60;

    $total_minutes_worked = $check_out_minutes - $check_in_minutes;
    ...
    if ($dayOfWeek == "土曜日" || $dayOfWeek == "日曜日") {
        if ($total_minutes_worked > 480) {
            // 休憩時間1時間を除く
            $total_minutes_worked -= 60;
            $holiday_overtime_minutes = $total_minutes_worked - 480;
        }
    }
    // 平日の残業
    else {
        // 残業時間の計算
        $over_minutes = $check_out_minutes - $overtime_start_time;
        // 22:00未満
        if ($over_minutes > 0 and $over_minutes < 180) {
            $overtime_minutes = $over_minutes;
        }
        // 22:00以降
        else if ($over_minutes >= 180) {
    ...
    if ($result->num_rows > 0) {
        ...
        $insert_stmt->bind_param("sssssiiiii", $id, $name, $date, $dayOfWeek, $workTime, $closeTime, $overtime_minutes, $late_night_overtime_minutes, $holiday_overtime_minutes, $overtime_pay);
        $insert_stmt->execute();
        echo "レコードを追加しました。";
    }

    echo "<br><br><hr><br><a href='login.php'>ログインへ戻る</a>";
    $con->close();
    ?>
</body>
